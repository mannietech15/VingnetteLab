...
import { Mail, Lock, Eye, EyeOff, Github, User, Sparkles } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const getStrength = () => {
    ...
    if (password.length < 6) return { score: 1, text: 'Weak', color: '#ef4444' };
    if (password.length < 10) return { score: 2, text: 'Fair', color: '#f59e0b' };
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#09090b', color: '#fafafa', fontFamily: 'var(--font-inter), sans-serif' }}>
      ...
      <div style={{ flex: '0 0 60%', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '60px', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 100% 0%, rgba(236,72,153,0.15) 0%, transparent 50%), radial-gradient(circle at 0% 100%, rgba(99,102,241,0.1) 0%, transparent 50%)', zIndex: 0 }} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} style={{ position: 'relative', zIndex: 10, maxWidth: '600px' }}>
          ...
          <div style={{ padding: '32px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px', backdropFilter: 'blur(10px)' }}>
            <p style={{ fontSize: '1.125rem', color: '#a1a1aa', fontStyle: 'italic', marginBottom: '24px', lineHeight: 1.6 }}>
              "The AI layout features completely changed how we do architecture planning. It's like having an extra team member."
            </p>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#ec4899', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600 }}>MW</div>
              <div>
                <div style={{ fontWeight: 600 }}>Marcus Webb</div>
                <div style={{ fontSize: '14px', color: '#71717a' }}>CTO, Linear</div>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      {/* Right Panel (40%) */}
      <div style={{ flex: '0 0 40%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px', position: 'relative', zIndex: 10, background: '#09090b', overflowY: 'auto' }}>
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} style={{ width: '100%', maxWidth: '400px' }}>
          
          <div style={{ marginBottom: '40px' }}>
            <h2 style={{ fontSize: '2rem', fontWeight: 700, letterSpacing: '-0.02em', marginBottom: '8px' }}>Create your account</h2>
            <p style={{ color: '#a1a1aa' }}>Start your free trial today</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div>
              <div style={{ position: 'relative' }}>
                ...
                <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required
                  ...
                  onFocus={e => { e.currentTarget.style.borderColor = '#6366f1'; e.currentTarget.style.boxShadow = '0 0 0 2px rgba(99,102,241,0.2)'; }}
                  onBlur={e => { e.currentTarget.style.borderColor = '#27272a'; e.currentTarget.style.boxShadow = 'none'; }} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '16px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#71717a', display: 'flex' }}>
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
              {password.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px', padding: '0 4px' }}>
                  <div style={{ flex: 1, height: '4px', borderRadius: '2px', background: strength.score >= 1 ? strength.color : '#27272a', transition: 'all 0.3s' }} />
                  <div style={{ flex: 1, height: '4px', borderRadius: '2px', background: strength.score >= 2 ? strength.color : '#27272a', transition: 'all 0.3s' }} />
                  <div style={{ flex: 1, height: '4px', borderRadius: '2px', background: strength.score >= 3 ? strength.color : '#27272a', transition: 'all 0.3s' }} />
                  <span style={{ fontSize: '12px', color: strength.color, width: '40px', textAlign: 'right', fontWeight: 600 }}>{strength.text}</span>
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '14px', color: '#a1a1aa' }}>
              <input type="checkbox" required style={{ accentColor: '#6366f1', width: '16px', height: '16px', cursor: 'pointer', marginTop: '2px' }} />
              <span>I agree to the <a href="#" style={{ color: '#6366f1', textDecoration: 'none' }}>Terms of Service</a> and <a href="#" style={{ color: '#6366f1', textDecoration: 'none' }}>Privacy Policy</a></span>
            </div>

            <button type="submit" disabled={isLoading || !email || !password || password !== confirmPassword}
              style={{
                ...
                cursor: (isLoading || !email || !password || password !== confirmPassword) ? 'not-allowed' : 'pointer',
                opacity: (isLoading || !email || !password || password !== confirmPassword) ? 0.7 : 1,
          ...
          <p style={{ textAlign: 'center', marginTop: '32px', color: '#a1a1aa', fontSize: '14px' }}>
            Already have an account? <Link href="/login" style={{ color: '#6366f1', textDecoration: 'none', fontWeight: 500 }}>Log in</Link>
          </p>
